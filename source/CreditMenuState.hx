...
class CreditMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...7)
		{
			bg = new FlxSprite(FlxG.width,0).loadGraphic(Paths.image('credits/bg'+i));
			bg.screenCenter();
			...
			credA = new FlxText(1800, 50, 0, "", 100);
			credA.ID = i;
			credA.font = Paths.font("FE-FONT.TTF");
			credA.bold = true;
			credA.fieldWidth = FlxG.width;
			grpCredA.add(credA);
			
			credB = new FlxText(1500, 212.5, 0, "", 50);
			credB.ID = i;
			...
			credA.updateHitbox();
			credA.alignment = FlxTextAlign.CENTER;
		}
	...
	function changeItem(huh:Int = 0)
	{
		...
		for (i in 0...grpBG.length)
		{
			if (i == curSelected)
			{
				...
				FlxTween.tween(grpCredA.members[i], {alpha: 1, x:0}, 1.5, {ease: FlxEase.expoInOut});
